...
def product_detail(request, product_id):
    ...
    aggregated_reviews = [
        review for review in aggregated_reviews
        if review.sentence_count > 2
    ]
    
    # Извлечение полных текстов отзывов через KeyThought
    for aggregated_review in aggregated_reviews:
        positive_reviews = aggregated_review.key_thoughts.filter(thought_type='positive').values_list('review__review_full_text', flat=True).distinct()
        negative_reviews = aggregated_review.key_thoughts.filter(thought_type='negative').values_list('review__review_full_text', flat=True).distinct()
