...
class Review(models.Model):
    ...
    corrected_text = models.TextField(
        verbose_name="Исправленный текст отзыва", null=True, blank=True
    )
    analysed_at = models.DateTimeField(
        null=True, blank=True, help_text="Дата и время последнего анализа отзыва"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class AggregatedReview(models.Model):
    ...
    product = models.ForeignKey(
        Product,
        on_delete=models.CASCADE,
        related_name="aggregated_reviews",
        verbose_name="Продукт",
    )
...
class KeyThought(models.Model):
    aggregated_review = models.ForeignKey(
        AggregatedReview,
        on_delete=models.CASCADE,
        related_name="key_thoughts",
        verbose_name="Агрегированный отзыв",
    )
...
class ProcessedReviewCache(models.Model):
    ...
    processed_data = models.JSONField(
        verbose_name="Обработанные данные", null=True, blank=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
